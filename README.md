...
- **100% Free Tier** - Uses AWS free tier resources
...
```
environments/
├── dev/dev.tfvars   → Development (VPC: 10.0.0.0/16)
└── prod/prod.tfvars  → Production (VPC: 10.1.0.0/16)
```
...
![Alt Text](results_outputs/plan.png)
...
![Alt Text](results_outputs/ec2.png)
...
├── 📁 Environments
│   ├── dev/
│   │   ├── dev.tfvars          # Development config overrides
│   │   └── variables.tf        # Dev-specific variable definitions
│   │
│   └── prod/
│       ├── prod.tfvars         # Production config overrides
